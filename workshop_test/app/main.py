...
def get_librarys(
    ...
    order: Optional[str] = Query(None, min_length=5, max_length=5),
...
def get_librarys_by_id(library_id: str = Path(None, min_length=5, max_length=5)): # เรากำหนดให้รหัสนองนักเรียนต้องเป็นตัวเลขเท่านั้น และต้องใส่มา10ตัวเท่านั้น
    try:
...
@app.post("/librarys")
def create_books(Library: createLibraryModel): #การสร้างข้อมูล
    try:
        library_id = mongo_db.create(library)
    except:
        raise HTTPException(status_code=500, detail="Something went wrong !!")
...
@app.patch("/Librarys/{library_id}") #ใช้ patch เพราะใช้ค้นหารหัสแค่บางคนเท่านั้น
def update_books(
    update_library: updateLibraryModel,
    library_id: str = Path(None, min_length=3, max_length=4),
):
...
def delete_book_by_id(library_id: str = Path(None, min_length=5, max_length=5)):
    try:
    ...
    if deleted_count == 0:
        raise HTTPException(
            status_code=404, detail=f"library Id: {deleted_library_id} is not Delete"  #การลบรหัสที่ผิด ให้เอารหัสมาใหม่
        )
